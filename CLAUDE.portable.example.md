...
- **Format + lint with `ruff`** — run `ruff format` and `ruff check` before committing (alongside `pytest`). `ruff` is in the lab dev dependencies and configured in `pyproject.toml` (it replaces black/flake8/isort); don't hand-format around it.
